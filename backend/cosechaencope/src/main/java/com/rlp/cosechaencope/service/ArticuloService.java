...
public class ArticuloService {
    ...
    /**
     * Obtiene múltiples artículos por sus IDs. Útil para cargar detalles de
     * varios artículos en una sola petición.
     *
     * @param ids Lista de IDs de artículos a obtener
     * @return Lista de ArticuloResponse con los artículos encontrados
     */
    public List<ArticuloResponse> obtenerArticulosPorIds(List<Long> ids) {
        log.info("Obteniendo {} artículos por IDs", ids.size());

        List<Articulo> articulos = articuloRepository.findAllById(ids);

        if (articulos.isEmpty()) {
            log.warn("No se encontraron artículos para los IDs proporcionados");
        }
